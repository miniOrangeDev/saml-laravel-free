...
if (isset($_POST['option']) && $_POST['option'] == 'mo_saml_trial_request') {

    if (!mo_saml_is_curl_installed()) {
        DB::update_option('mo_saml_message', 'ERROR: <a href="http://php.net/manual/en/curl.installation.php" target="_blank">PHP cURL extension</a> is not installed or disabled. Use case submit failed.');
        mo_saml_show_error_message();
        return;
    }

    $email = $_POST['mo_saml_trial_request_email'];
    $use_case = $_POST['mo_saml_trial_request_use_case'];

    if (mo_saml_check_empty_or_null($email) || mo_saml_check_empty_or_null($use_case)) {
        DB::update_option('mo_saml_message', 'Please fill up Email and Use Case fields to submit your request.');
        mo_saml_show_error_message();
        ...
        if ($submited == false) {
            DB::update_option('mo_saml_message', 'Your use case could not be submitted. Please try again.');
            mo_saml_show_error_message();
        } else {
